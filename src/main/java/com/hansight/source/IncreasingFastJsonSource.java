 ...
 * @description .
 */
public class IncreasingFastJsonSource extends BaseSourceWithKeyRange<JSONObject> {
    ...
    public IncreasingFastJsonSource(int numEvents, int numKeys) {
    ...
    protected JSONObject getElement(int keyId) {
        try {
            ...
            int idx = cnt.get() % 0xFF;
            element.put("event_name", "邮件登陆");
            element.put("src_address", "172.16.100." + idx);
            element.put("dst_address", "172.16.100." + idx);
